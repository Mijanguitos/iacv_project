...
if __name__ == "__main__":
    ...
    PROJECT_ROOT = f"{Path().resolve()}\\src"

    clip = "clip_7"
    video_path = f"{PROJECT_ROOT}\\ball_detection\\preprocessing_output\\preprocessed_{clip}"
    json_path = f"{PROJECT_ROOT}\\spin\\spin_output\\{clip}_spin"
    save_path = f"{PROJECT_ROOT}\\spin\\postprocessing_output\\{clip}"
    trajectory_path = f"{PROJECT_ROOT}\\ball_detection\\postprocessing_output\\postprocessed_{clip}.json"

    spin_post_processing(json_path, save_path, video_path)
